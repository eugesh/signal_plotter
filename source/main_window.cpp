...
MainWindow::estimate_contour_params() {
    ...
    for(unsigned int i = end_index; i < samples_attenuation.size(); ++i) {
...
// void signal_analyzer(double *a, double *b, Samples const& data, double *q_factor, double *freq, double first, double step, unsigned int r_end_i) {
    ...
    Samples signal = Samples(samples_attenuation.begin() + end_index, samples_attenuation.end());

    Intervals zero_intervals = find_all_zeros_indices(signal);

    Peaks all_peaks = find_all_peaks(signal, zero_intervals);

    Peaks real_peaks = find_real_peaks(signal, all_peaks, 0.05);

    *freq = estimate_frequency(real_peaks, graph_attenuation.xOffset, graph_attenuation.xScale);

    *q_factor = estimate_quality(signal, real_peaks);
    estimate_quality_ls(a, b, signal, real_peaks, graph_attenuation.xOffset, graph_attenuation.xScale, end_index);
