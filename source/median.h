...
void median1d(float *out, float *in, int msize);
 ...
 */
void
median1d(float *out, float *in, unsigned int l, unsigned int msize) {
  ...
  for(unsigned int i = shift; i < l - shift; ++i) {
    ...
    // Sort mask.
    qsort(mask, sizeof(mask)/sizeof(*mask), sizeof(*mask), compare);
